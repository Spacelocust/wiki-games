    ...
    return (
        <div>
            <div className='d-flex justify-content-between'>
                <BadgeComponent className='font-small text-dark border-dark bg-light mx-2' position='end' onClick={() => setMore(!more)}><IconSetter icon={faEllipsisH} /></BadgeComponent>
                <BadgeComponent className='font-small'>{ status }</BadgeComponent>
            </div>
            <div className="d-flex align-items-center justify-content-evenly">
                <div className="d-flex align-items-center flex-column w-50">
                    <img src={match.opponents[0].opponent.image_url} alt="" style={{ height: '3rem', width: '3rem' }}/>
                    <p className="text-center m-0">{match.opponents[0].opponent.name}</p>
                </div>
                <div>
                    <CharmSwords height={'48px'}/>
                    <p className='text-center my-1'>{`${match.results[0].score} - ${match.results[0].score}`}</p>
                </div>
                <div className="d-flex align-items-center flex-column w-50">
                    <img src={match.opponents[1].opponent.image_url} alt="" style={{ height: '3rem', width: '3rem' }}/>
